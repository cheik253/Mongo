...
def login():
    ...
    if request.method == 'POST':
        ...
        # Assuming Voter is your collection name
        user = Voter.find_one({'name': name, 'paswd': password})

        if user:
            session.permanent = True  # Set session as permanent
            session['user'] = name
            if user.get('type') == 'admin':  # Check if user is admin
                return render_template('admin.html')
            else:
                return redirect(url_for('dashboard'))  # Redirect to dashboard if user is not admin
        else:
