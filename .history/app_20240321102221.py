...
def dashboard():
    ...
    M=db.Candidate.aggregate([
    {
        '$project': {
            'name': 1, // Keep the 'name' field
            'numVoters': { '$size': '$voters' } // Compute the size of the 'voters' array
        }
    },
    {
        '$group': {
            '_id': '$name', // Group by candidate name
            'totalVoters': { '$sum': '$numVoters' } // Sum the number of voters for each candidate
        }
    }
])

    
    return render_template('voter_dashboard.html',V=Voter,C=Candidate,M=M)
