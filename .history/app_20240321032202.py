from flask import Flask, render_template, jsonify,url_for,redirect,request,form,flash
...
def register():
    if request.method == 'POST':
        ...
        if age < 16:
            flash('Age should be greater or equal to 16.')
            return redirect(url_for('register'))

        query = {'name': nom, 'age': age, 'has_voted': 0, 'email': email, 'paswd': pawd}
        mongo.db.Voter.insert_one(query)  # Assuming 'Voter' is your collection
